...
namespace CableOverTray
{
    ...
    public class Command : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            application1.Application.FailuresProcessing += new EventHandler<FailuresProcessingEventArgs>(this.DoFailureProcessing);
            ...
            ExternalEvent rste = ExternalEvent.Create((IExternalEventHandler)new RunSingleTrayHandeler());
            ExternalEvent sste = ExternalEvent.Create((IExternalEventHandler)new CableOverTray.SelectSortedTrays());
            ExternalEvent rmte = ExternalEvent.Create((IExternalEventHandler)new RunMultibleTraysHandeler());
            ExternalEvent rcste = ExternalEvent.Create((IExternalEventHandler)new RunCustomSingleTrayHandeler());
            ExternalEvent scte = ExternalEvent.Create((IExternalEventHandler)new SelectCustomTray());
            ExternalEvent scce = ExternalEvent.Create((IExternalEventHandler)new SelectCustomConds());
            ExternalEvent rmcte = ExternalEvent.Create((IExternalEventHandler)new RunMultibleCustomTraysHandeler());
            IList<Element> elements1 = new FilteredElementCollector(document).OfCategory(BuiltInCategory.OST_Conduit).WhereElementIsElementType().ToElements();
            ...
            mainWindow.Show();
            //if (ApplicationStatic_DB.MainForm == null)
            //{
            //  ApplicationStatic_DB.MainForm = new COT_Form_Main(rste, sste, rmte, rcste, scte, scce, rmcte, types);
        ...
        public List<CableTray> SelectSortedTrays(Autodesk.Revit.UI.Selection.Selection sel, UIDocument uidoc, Document doc)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    Element element = doc.GetElement(sel.PickObject(ObjectType.Element, string.Format("Selected CableTrays: {0} Trays", (object)cableTrayList.Count)));
                    int index = 0;
                    bool flag = false;
